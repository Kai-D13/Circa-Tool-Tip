...
```js
chrome.runtime.sendMessage(chrome.runtime.id, { v: 1, type: "GET_SYNC_STATUS" }, console.log)
```

| # | Thao tác | Kỳ vọng | Kết quả |
|---|---|---|---|
| 1.1 | Chạy lệnh trên | `sites.pos.state = "no-release"`, `sites.admin.state = "no-release"`, `revision: 0` | |
| 1.2 | Nhìn Console | **Không** có lỗi validate nào. Chưa phát hành là bình thường, không phải hỏng | |
| 1.3 | `chrome.storage.local.get(null, console.log)` | Có `releaseStatus:pos` / `releaseStatus:admin`, **không** có `release:pos` / `release:admin` | |
...
| 2.3 | Console service worker: `chrome.runtime.sendMessage(chrome.runtime.id, { v: 1, type: "SYNC_NOW" }, console.log)` | `ok: true`, `sites` có `{ site: "pos", action: "updated", revision: 1 }` | |
| 2.4 | `chrome.storage.local.get("release:pos", console.log)` | Có payload đầy đủ: `schemaVersion: 5`, `site: "pos"`, `revision: 1`, mảng `guides` | |
| 2.5 | `chrome.storage.local.get("releaseStatus:pos", console.log)` | `state: "ok"`, `revision: 1`, `checksum` khớp với cái Portal hiện ở 2.1 | |
...
| # | Thao tác | Kỳ vọng | Kết quả |
|---|---|---|---|
| 3.1 | Chạy `SYNC_NOW` lần nữa ngay lập tức | `action: "unchanged"` cho POS | |
| 3.2 | Mở tab **Network** của service worker rồi chạy lại | Chỉ có request tới `release_heads`. **Không** có request `get_release` | |
...
| 4.2 | Chạy `SYNC_NOW` | `action: "updated"`, `revision: 2` | |
| 4.3 | `chrome.storage.local.get("release:pos", console.log)` | `revision: 2`, và số guide tăng đúng | |
...
| 5.2 | Chạy `SYNC_NOW` | `ok: false`, mỗi site có `action: "error"` | |
| 5.3 | `chrome.storage.local.get("release:pos", console.log)` | **Payload vẫn còn nguyên, `revision: 2`** | |
| 5.4 | `chrome.storage.local.get("releaseStatus:pos", console.log)` | `state: "error"`, nhưng `revision: 2` — trạng thái vẫn trỏ vào bản đang thật sự dùng | |
| 5.5 | Bỏ **Offline**, chạy `SYNC_NOW` | Trở lại `state: "ok"` | |

## 6. Sống sót qua service worker và qua restart

| # | Thao tác | Kỳ vọng | Kết quả |
|---|---|---|---|
| 6.1 | Để yên ~40s cho service worker ngủ, rồi chạy `GET_SYNC_STATUS` | Vẫn đọc được `revision: 2` | |
| 6.2 | Đóng hẳn Chrome, mở lại, chạy `GET_SYNC_STATUS` | Vẫn `revision: 2` — cache nằm ở `storage.local` nên sống qua restart | |
